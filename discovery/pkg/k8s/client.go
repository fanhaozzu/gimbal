...
// in-cluster config.
func NewClient(masterUrl string, kubeCfgFile string, logger *logrus.Logger) (kubernetes.Interface, error) {
	config, err := buildConfig(masterUrl, kubeCfgFile, logger)
	if err != nil {
		return nil, err
	}
...
func NewClientWithQPS(masterUrl string, kubeCfgFile string, logger *logrus.Logger, qps float32, burst int) (kubernetes.Interface, error) {
	config, err := buildConfig(masterUrl, kubeCfgFile, logger)
	if err != nil {
		return nil, err
	}
